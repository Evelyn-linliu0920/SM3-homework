...
// 此函数专门解决Windows环境下控制台中文显示乱码问题
// 通过设置控制台代码页和字体，确保中文字符正确显示
static void init_windows_console() {
    // 1. 输入输出编码设为GBK（代码页936，Windows默认中文编码）
    // 这是修复中文显示的关键步骤，确保控制台能正确处理中文字符
    SetConsoleCP(936);
    SetConsoleOutputCP(936);

    // 2. 配置控制台字体为"新宋体"（避免字体导致的中文乱码）
    // 某些Windows系统默认字体不支持中文，需要手动指定中文字体
...
// 比较两个哈希值是否相同（碰撞检测用）
// 此函数逐字节比较两个256位哈希值，用于检测是否发生哈希碰撞
...
// 由于Visual Studio将strdup标记为不安全，这里提供自定义实现
...
// 执行国家密码管理局指定的标准测试用例，验证SM3算法的正确性
...
// 测试SM3算法在边界条件下的表现，包括空文件、单字节、特定长度等特殊场景
...
// 测试SM3算法的抗碰撞性，生成大量随机输入，检查是否有不同输入产生相同哈希值
...
// 测试SM3算法的雪崩效应，检查输入微小变化是否导致哈希值巨大变化
...
// -------------------------- 主函数（调用控制台初始化，解析命令行） --------------------------
// 程序的入口点，负责初始化环境、解析命令行参数并调用相应的测试函数
